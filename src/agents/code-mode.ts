...
export function createCodeModeTools(ctx: CodeModeToolContext): AnyAgentTool[] {
  const execTool = markCodeModeControlTool({
    ...
    parameters: Type.Object({
      ...
      code: Type.String({
        description:
          'Required JS/TS; no Python, shell, `require`, `import`. Use `return value`; a trailing expression is discarded and yields `null`. Use `callValue`, not `call`. Declared schemas prove structure only: transform deterministically inline; before a mutation needing entity identity, uniqueness, permission, or interpretation, return candidates; act in a later exec. Unknown: return it first, then parse it in a later exec. Read: `const file=await tools.callValue("openclaw:core:read", { path: "notes.txt" }); return file.content;`. Use exact ids from `ALL_TOOLS` or `tools.search(query)`; never parallelize dependent calls.',
      }),
    ...
    execute: async (
      ...
      const result = normalizeCodeModeTimeoutResult(
        await runExec({
          ...
          restartSafe: ctx.forceRestartSafeTools === true,
          signal,
          onUpdate,
          onRuntime: (value) => {
            runtime = value;
